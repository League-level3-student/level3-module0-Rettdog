...
sparks=new Spark[100];
		...
		int xVel = rand.nextInt(20)-10;
		int yVel = rand.nextInt(10)-40;
	...
	for(int i = 0;i<sparks.length;i++) {
		sparks[i]=new Spark(FireworkDisplay.WIDTH/2,FireworkDisplay.HEIGHT,xVel,yVel);
	}
	}
	...
	public void launch() {
		...
		for(Spark spark:sparks) {
			spark.x=FireworkDisplay.WIDTH/2;
			spark.y=FireworkDisplay.HEIGHT;
		}
	}
	...
	public void drawSparks(Graphics g) {
		...
		for(Spark spark:sparks) {
			//System.out.println(spark.x+"\n"+spark.y+"\n"+spark.size);
			g.setColor(spark.color);
			g.fillOval(spark.x,spark.y,spark.size,spark.size);
		}
